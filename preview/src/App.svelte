  ...
  onMount(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        await scan(true);
        status = 'Checking missing game information…';
        await invoke<number>('refresh_metadata', { gameId: null });
        await refresh();
...
<main>
  ...
  {#if selectedGame}
    <section id="achievement" aria-label={`${selectedGame.name} achievements`}>
      ...
      <div class="achievement-tools"><div class="achievement-sort"><button class:active={achievementSort === 'name'} title="Sort achievements alphabetically" aria-label="Sort achievements alphabetically" onclick={() => achievementSort = 'name'}><i class="fas fa-sort-alpha-down"></i></button><button class:active={achievementSort === 'time'} title="Sort by unlock time" aria-label="Sort by unlock time" onclick={() => achievementSort = 'time'}><i class="far fa-clock"></i></button><button class:active={achievementSort === 'progress'} title="Sort by progress" aria-label="Sort by progress" onclick={() => achievementSort = 'progress'}><i class="fas fa-percent"></i></button><button class:active={achievementSort === 'rarity'} title="Sort by global rarity" aria-label="Sort by global rarity" onclick={() => achievementSort = 'rarity'}><i class="fas fa-gem"></i></button></div><div id="achievement-search"><span><i class="fas fa-search"></i></span><input type="search" bind:value={achievementQuery} placeholder="Search achievements" aria-label="Search achievements" /></div></div>
      ...
      {#each [['Unlocked', true], ['Locked', false]] as group}
        ...
        {#if rows.length || group[1] || hiddenLockedCount() > 0}
          <section class="achievement-group">
            <h3><span><i class={group[1] ? 'fas fa-unlock' : 'fas fa-lock'}></i> {group[0]} <small>{groupCount}</small></span><button class:active={!collapsed} aria-label={`${collapsed ? 'Expand' : 'Collapse'} ${group[0]} achievements`} onclick={() => group[1] ? unlockedCollapsed = !unlockedCollapsed : lockedCollapsed = !lockedCollapsed}><i class="fas fa-chevron-right"></i></button></h3>
            {#if !collapsed}<ul>
  ...
  {:else}
    <section id="home" aria-labelledby="library-title">
      <div id="user-info"><button class="avatar" class:squared={settings?.profileAvatarSquared} title="Choose profile avatar (right-click for options)" aria-label="Choose profile avatar" onclick={chooseAvatar} oncontextmenu={(event) => { event.preventDefault(); event.stopPropagation(); avatarMenu = { x: event.clientX, y: event.clientY }; }}><img src={avatarData || defaultAvatar} alt="" /></button><div class="info"><h1>{settings?.username || 'Achievement Watcher'}</h1><ul><li><i class="fas fa-trophy"></i> <strong>{totalUnlocked()}</strong> unlocked</li><li><i class="fas fa-gamepad"></i> <strong>{games.length}/{completedGames()}</strong> games</li><li><i class="fas fa-cookie-bite"></i> <strong>{averageCompletion()}%</strong> average</li></ul>{#if trophyTotal('platinum') + trophyTotal('gold') + trophyTotal('silver') + trophyTotal('bronze') > 0}<ul class="trophy-totals" aria-label="PlayStation trophies"><li class="platinum" title="Platinum trophies"><i class="fas fa-trophy"></i> <strong>{trophyTotal('platinum')}</strong></li><li class="gold" title="Gold trophies"><i class="fas fa-trophy"></i> <strong>{trophyTotal('gold')}</strong></li><li class="silver" title="Silver trophies"><i class="fas fa-trophy"></i> <strong>{trophyTotal('silver')}</strong></li><li class="bronze" title="Bronze trophies"><i class="fas fa-trophy"></i> <strong>{trophyTotal('bronze')}</strong></li></ul>{/if}</div></div>
      <div class="library-tools"><div id="sort-box" aria-label="Sort games"><button class:active={librarySort === 'name'} title="Sort alphabetically" aria-label="Sort alphabetically" onclick={() => librarySort = 'name'}><i class="fas fa-sort-alpha-down"></i></button><button class:active={librarySort === 'progress'} title="Sort by completion" aria-label="Sort by completion" onclick={() => librarySort = 'progress'}><i class="fas fa-sort-numeric-down"></i><i class="fas fa-percent"></i></button><button class:active={librarySort === 'recent'} title="Sort by most recent unlock" aria-label="Sort by most recent unlock" onclick={() => librarySort = 'recent'}><i class="fas fa-sort-numeric-down"></i><i class="far fa-clock"></i></button></div><div id="search-bar"><span><i class="fas fa-search"></i></span><input type="search" bind:value={query} placeholder="Search games" aria-label="Search games" /></div><select bind:value={libraryFilter} aria-label="Filter games"><option value="all">All games</option><option value="tracked">Tracked</option><option value="cached">Cached information</option></select><button class="refresh" title="Refresh library" aria-label="Refresh library" onclick={() => scan(false)} disabled={scanning}><i class="fas fa-sync-alt"></i></button></div>

            ...
            {#if !collapsed}<ul>
              ...
              {#each rows as achievement}
                <li><article data-achievement-id={achievement.achievementId} class:highlight={highlightedAchievement === achievement.achievementId} class:unlocked={achievement.achieved} class:rare={(achievement.globalPercentHundredths ?? 10_001) <= 1000} class="achievement-row">
                  ...
                  <div class="achievement-state">{#if achievement.originSourceId}<span class="achievement-origin-label" title={sourceDescription(kindForSource(achievement.originSourceId))}><i class="achievement-origin source-badge">{#if sourceIcon(kindForSource(achievement.originSourceId))}<img src={sourceIcon(kindForSource(achievement.originSourceId))!} alt="" />{:else}{sourceMark(kindForSource(achievement.originSourceId))}{/if}</i>{sourceLabel(kindForSource(achievement.originSourceId))}</span>{/if}{#if achievement.trophyGrade}<i class={`trophy-grade ${achievement.trophyGrade} fas fa-trophy`} title={`${achievement.trophyGrade} trophy`}></i>{/if}{#if achievement.achieved}<strong>{t('unlocked', 'Unlocked')}</strong>{#if achievement.unlockTime > 0}<time title={new Date(achievement.unlockTime * 1000).toLocaleString()}>{formatUnlockTime(achievement.unlockTime)}</time>{/if}{:else}<span>{t('locked', 'Locked')}</span>{/if}{#if achievement.globalPercentHundredths !== undefined}<small title="Global Steam unlock percentage"><i class="fas fa-gem"></i> {(achievement.globalPercentHundredths / 100).toFixed(2)}% {t('globalStat', 'of players have this')}</small>{/if}</div>
                </article></li>
              {/each}
